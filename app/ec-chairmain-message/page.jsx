...
export default function EcChairmanMessagePage() {
  ...
  useEffect(() => {
    const fetchChairmanData = async () => {
      try {
        const response = await fetch('https://methodical-kindness-fc585984ed.strapiapp.com/api/ec-chairman-message?populate=*');
        ...
        setError("Failed to fetch EC Chairman data");
        console.error("Error fetching EC Chairman data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="animate-pulse text-[#017381] text-xl">Loading EC Chairman message...</div>
      </div>
    );
  }
  ...
  if (!chairmanData) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="text-[#017381] text-xl">No EC Chairman data available</div>
      </div>
    );
  }
