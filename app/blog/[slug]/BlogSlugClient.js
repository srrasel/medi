    ...
    const fetchBlogPost = async () => {
      try {
        ...
        const item = posts.find(
          (post) =>
            String(post.slug || post.id) === String(slug) ||
            String(post.id) === String(slug) ||
            String(post.Title || "")
              .toLowerCase()
              .replace(/[^a-z0-9]+/g, "-") === String(slug),
        )
        ...
        setViewCount(transformedPost.views)
        setShareCount(45)
      } catch (e) {
    ...
    const fetchAllBlogPosts = async () => {
      try {
        ...
        const transformedPosts = Array.isArray(data)
          ...
          : []
        if (blogPost) {
          const filtered = transformedPosts.filter((post) => post.id !== blogPost.id)
          setRelatedPosts(filtered.slice(0, 3))
        } else {
          setRelatedPosts(transformedPosts.slice(0, 3))
        }
      } catch (e) {
